...
public class ZipUtils {
		...
		try (MyZipFile zipFile = new MyZipFile(File)) {
			...
			while (entries.hasMoreElements()) {
				...
				if (zipEntry.isDirectory()) {
					...
					if (!newDir.mkdirs()) { throw new RuntimeException("Creation of target dir was failed, target dir: " + zipDestinationFolder + ", entity: " + name); }
				} else {
	...
	public static File createTargetFile(File zipDestinationFolder, String name) {
		...
		if (name.endsWith(File.separator)) {
			if (!destinationFile.isDirectory() && !destinationFile.mkdirs()) { throw new RuntimeException("Error creating temp directory:" + destinationFile.getPath()); }
			...
			if (!parentFolder.isDirectory()) {
				if (!parentFolder.mkdirs()) { throw new RuntimeException("Error creating temp directory:" + parentFolder.getPath()); }
			}
		}
		return destinationFile;
	}
